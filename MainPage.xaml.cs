...
public partial class MainPage : ContentPage
{
    ...
    private const string PositionPrefix =
        "PhoneGamepad_Position_";
...
private void EnableAndroidMultiTouch()
{
    ...
    private void Joystick_PanUpdated(
        object? sender,
        PanUpdatedEventArgs e)
    {
        ...
        switch (e.StatusType)
        {
            case GestureStatus.Running:
                ...
                gamepad.LX =
                    (float)Math.Clamp(
                        (x / JoystickRadius) / JoystickFullThreshold,
                        -1.0,
                        1.0);

                gamepad.LY =
                    (float)Math.Clamp(
                        (y / JoystickRadius) / JoystickFullThreshold,
                        -1.0,
                        1.0);
    ...
    private void RightJoystick_PanUpdated(
        object? sender,
        PanUpdatedEventArgs e)
    {
        if (isEditMode)
    ...
    // 判断触摸点属于哪个控件
    //
    // point 是相对于 GamepadLayout 的坐标
    // =========================================================

    private View? FindViewAtPoint(
        double x,
        double y)
    {
        // 从后往前检查。
        // 后面的控件视觉层级更高。
        View[] views =
            GetEditableViews().ToArray();


        for (int i = views.Length - 1;
             i >= 0;
             i--)
        {
            View view = views[i];
...
private class EditDragTouchListener :
    Java.Lang.Object,
...
{
    if (EditDragLayer.Handler?.PlatformView
        is not Android.Views.View nativeLayer)
    {
        return;
    }

    editDragTouchListener ??=
        new EditDragTouchListener(this);

    nativeLayer.SetOnTouchListener(
        editDragTouchListener);
}
...
{
    if (EditDragLayer.Handler?.PlatformView
        is Android.Views.View nativeLayer)
    {
...
private void BeginEditDrag(
    float rawX,
    float rawY)
{
    draggingView = null;

    View? target = FindViewAtScreenPoint(
        rawX,
        rawY);
    ...
    dragStartTranslationX =
        target.TranslationX;

    dragStartTranslationY =
        target.TranslationY;
}
...
private View? FindViewAtScreenPoint(
    float rawX,
    float rawY)
{
    ...
    for (int i = views.Length - 1; i >= 0; i--)
    {
        ...
        float right =
            left + nativeView.Width;

        float bottom =
            top + nativeView.Height;
    ...
    float rawY)
{
    ...
    double deltaX =
        (rawX - dragStartRawX) / density;

    double deltaY =
        (rawY - dragStartRawY) / density;
    ...
    draggingView.TranslationX =
        dragStartTranslationX + deltaX;

    draggingView.TranslationY =
...
private void EndEditDrag()
{
    ...
    private void EditLayoutButton_Clicked(
        object sender,
        EventArgs e)
    {
        EnableEditMode();
    ...
    private void EnableEditMode()
    {
        ...
        SaveLayoutButton.IsVisible = true;
    ...
    private void SaveLayoutButton_Clicked(
        object sender,
        EventArgs e)
    {
        SaveCurrentLayout();
...
private void ResetToFactoryLayout()
    {
        ...
        // 恢复程序最初的标准布局
        //
        // XAML 中的 AbsoluteLayout.LayoutBounds
        // 就是我们的标准位置。
        //
        // 用户编辑时，我们只修改 TranslationX / TranslationY。
        // 所以恢复标准布局时，把所有 Translation 清零即可。
        // =========================================================
    ...
    private void FinishEditButton_Clicked(
        object sender,
        EventArgs e)
    {
        DisableEditMode();
    ...
    private void DisableEditMode()
    {
        ...
        SaveLayoutButton.IsVisible = false;
